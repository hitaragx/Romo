...
@property (nonatomic, assign) dispatch_queue_t writer_queue;
...
- (AVCWriterObj*) setupWriterObj {
    ...
    NSString* sid = (NSString*) CFUUIDCreateString(NULL, uid);
    ...
    wobj.writer = [[[AVAssetWriter alloc] initWithURL:[NSURL URLWithString:fileurl] fileType:AVFileTypeQuickTimeMovie error:&err] autorelease];
    ...
    wobj.pixelAdaptor = [[[AVAssetWriterInputPixelBufferAdaptor alloc] initWithAssetWriterInput:wobj.writerInput sourcePixelBufferAttributes:pbp] autorelease];
    ...
    AVCEncoderBlock block = ^(CMSampleBufferRef sample){
        ...
        switch (self.writerObj->writer.status) {
            ...
            case AVAssetWriterStatusWriting:
                ...
                @synchronized(self->timeQueue) {
                    [self->timeQueue addObject:[[[AVCTimeObj alloc] initWithTime:startTime] autorelease]];
                }
...
- (void) stop {
    ...
    dispatch_release(self.writer_queue);
}
